...
type VisaOrder = {
  ...
  route: string;
  backgroundImageUrl: StaticImageData | string;
};

const VisaSection: FC<VisaOrder> = ({ title, flag, route, backgroundImageUrl }) => {
  const { push } = useRouter();
  const visatracking = new VisaTracking();
  const visaObject: IVisa = { visaTitle: title, brand: route };
  return (
    <div
    ...
    >
      <div className={styles['visa-container__visa-section__content']}>
        <div className={styles['visa-container__visa-section__content__title']}>
          <Image className="rounded-2" src={flag} alt="visa" width={80} height={50} />
          <span>{title}</span>
        </div>
        <div>
          <Button
