...
export type LandingPageType = {
  content: TFlightContents['data'][number]['attributes'];
  breadCrumb: Record<string, never>;
  path: string;
};

export type NewLandingPageType = {
  content: DataApiResponse;
  breadCrumb: Record<string, never>;
  path: string;
};
export default function Bus({ content }: NewLandingPageType) {
  const isSuperApp = useIsSuperApp();
  return <>{!isSuperApp && cmsFactory({ cmsData: content })}</>;
}
Bus.getLayout = function getLayout(page: ReactElement) {
