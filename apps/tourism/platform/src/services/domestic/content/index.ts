...
export const getDomesticLandingContent = async (query_id: string) => {
  ...
  const url = `${process.env.NEXT_PUBLIC_CMS_DOMAIN}api/seo-contents?filters[origin_id][$eq]=${
    ...
    routeId[1],
  ]}&populate=faq,meta_seo.twitter.image,meta_seo.metaOgSocial.image&filters[service][$eq]=flights`;
